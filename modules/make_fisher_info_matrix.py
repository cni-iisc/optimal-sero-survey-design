      ...
      for j in range(3):
          if(x[j] == "0.0"):
              for i in range(3):
                  ...
                  prod[i] = (prod[i])*(self.highs[i, j]*(1-self.Sen[j]) + (1-self.highs[i, j])*self.Spec[j])

          if(x[j] == "1.0"):
              for i in range(3):
                  # See explanation above
                  prod[i] = prod[i]*(self.highs[i, j]*(self.Sen[j]) + (1-self.highs[i, j])*(1 - self.Spec[j]))

      r = np.append(p, 1-sum(p))
      prod = np.array(prod)
      # log-probability, gradients[3], probability
      return [np.log(prod.dot(r)), prod[0]-prod[2], prod[1]-prod[2],  prod.dot(r)]
