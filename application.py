...
def init():
    ...
    if request.method == "POST":
        ...
        senRAT = float(request.form.get('senRAT'))
        specRAT = float(request.form.get('specRAT'))

        cRTPCR = request.form.get('cRTPCR')
        senRTPCR = float(request.form.get('senRTPCR'))
        specRTPCR = float(request.form.get('specRTPCR'))

        cIgG = request.form.get('cIGG')
        senIgG = float(request.form.get('senIGG'))
        specIgG = float(request.form.get('specIGG'))

        if not((0 < senRAT < 1) and (0 < senRTPCR < 1) and (0 < senIgG < 1) and \
        (0 < specRAT < 1) and (0 < specRTPCR < 1) and (0 < specIgG < 1)):
            return render_template('index.html', m=("%.5f" % marginError), mCost=budget, totalCost=totalCost, column_names=res.columns.values, row_data=list(res.values.tolist()), zip=zip, message="Sensitivities or specifities must be strictly between 0 and 1. We have reset the form with default inputs.")
        ...
        if method == 'local': # run locally-optimal design
            ...
            res, objFunVal = local_optimization(cRAT, cRTPCR, cIgG, p_vec,specRAT=specRAT, specRTPCR=specRTPCR,
              specIGG=specIgG, senRAT=senRAT, senRTPCR=senRTPCR, senIGG=senIgG)
        ...
        if method == 'grid': # run exhaustive search for worst-case design
            ...
            p3_start = p3[0]
            p3_end = p3[1] + 0.01
            res, objFunVal = exhaustive_search(cRAT, cRTPCR, cIgG, p1_start, p1_end, p2_start, p2_end, p3_start, p3_end,specRAT=specRAT, specRTPCR=specRTPCR,
              specIGG=specIgG, senRAT=senRAT, senRTPCR=senRTPCR, senIGG=senIgG)
